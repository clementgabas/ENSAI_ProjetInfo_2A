...
def verif_entier(x):
    ...
    """
    Cette fonction prend en argument une chaine de caracteres.
    En effet, les inputs sont codes en tant que tels.
    """
    ...
    try:
        ...
        print("Erreur : Le nombre saisi n'est pas entier!")
        return(False)
    
    
    

def verif_entier_bornes(x, a, b):

    """
    Cette fonction prend en argument un nombre sous la forme d'un string (format de l'input)
    Elle verifie que cet entier est compris entre les bornes a et b (incluses).
    """

    testeur = verif_entier(x)
    ...
    if testeur :
        ...
        if ((a <= x) and (x <= b)) : return(True)
            ...
            print("Erreur : le nombre n'est pas compris entre {} et {}".format(a, b))
            return(False)
...
def resume_info(self):
    ...
    """
    Cette fonction va être découpée en sous fonction, chacune correspondant aux choix
    possibles de faire. Elle fait donc office de menu.
    """
    ...
    while test == False:
        ...
        choice = input("[1] : Afficher les informations de certains pays \n"
                       + "[2] : Afficher les n premiers/derniers pays dont un critère est le plus important \n"
                       + "[3] : Afficher les pays dont un critère dépasse (ou ne dépasse pas) un certain seuil. \n"
                       + "[4] : Afficher le tableau des classes d’âge pour certains pays (10 pays au maximum) \n"
                       + "[5] : Résumé statistique d'un critère \n"
                       + "[6] : Valeurs extrêmes d'un critère \n"
                       + "[7] : Afficher les dépenses de certains pays \n"
                       + "[8] : Retourner au menu \n")

        test = verif_entier_bornes(choice, 1, 8)

    choice = int(choice)

    if choice == 1:
        return(affiche_infos(self))

    if choice == 2:
        return(affiche_max_critere(self))

    if choice == 3:
        return(critere_seuil(self))

    if choice == 4:
        return(tableau_classe(self))

    if choice == 5:
        return(stat_desc(self))

    if choice == 6:
        return(valeur_extreme(self))

    if choice == 7:
        return(tableau_depense(self))

    if choice == 8:
        print("Redirection vers le menu.") #Retour vers menu
'''
